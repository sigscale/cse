%%% mod_cse_rest_accepted_content.erl
%%% vim: ts=3
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%% 	managed in modules named `cse_rest_res_*'.
%%%
...
%%% @end
%%%
-module(mod_cse_rest_accepted_content).
				...
				undefined ->
					...
					case string:lexemes(Path, [$/]) of
						...
						["resourceInventoryManagement", "v4", "resource" | _] ->
							check_content_type_header(cse_rest_res_resource, ModData);
						["nrf-rating", "v1", "notify", _] ->
							check_content_type_header(cse_rest_res_nrf, ModData);
