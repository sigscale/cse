...
%%
suite() ->
	Description = "Test suite for Intelligent Network Application Part (INAP) in CSE",
...
start_dialogue(Config) ->
	...
	{ok, {Balance, _}} = gen_server:call(OCS, {add_subscriber, SN, Balance}),
...
pdu_o_abandon(OTID, DTID, InvokeID) ->
	...
	TV2 = binary:part(cse_codec:date_time(calendar:universal_time()), 1, 6),
...
pdu_t_abandon(OTID, DTID, InvokeID) ->
	...
	Invoke2 = #{invokeId => {present, InvokeID + 1},
			opcode => ?'opcode-applyChargingReport',
			argument => ChargingResultArg},
	TV1 = binary:part(cse_codec:date_time(calendar:universal_time()), 1, 6),
